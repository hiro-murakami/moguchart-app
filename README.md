...
- **高速な仮想スクロール** — 公式 Vue 3 ラッパー（`@mogura/moguchart-vue`）およびコアライブラリ（`@mogura/moguchart-core`）を採用し、大量のタスクや行があっても滑らかに動作
...
- **コア統合による表示倍率（ズーム）とフォントサイズ連動** — 50%〜200%の全体スケーリングをコアチャートへ統合。カレンダー横幅だけでなく行ヘッダー幅、バーの高さ、フォントサイズ（タスク名・行名・ラベル・バッジ等の一括スケーリング）が自然に連動。Chrome風ズームUI（ステップ伸縮、ワンクリックリセット、ショートカット `Cmd+0` / `Ctrl+0`）、ホイール操作（Ctrl/Cmd）によるスムーズな双方向ズーム
...
- **強力な Undo / Redo（元に戻す・やり直し）** — コアライブラリの Command パターンおよび履歴管理システムと完全統合。タスク移動・リサイズ・進捗率ドラッグ変更・一括移動、タスク作成・編集・削除、行の追加・編集・削除・並び替え・階層操作（インデント/アウトデント）・一括展開/折りたたみ、依存関係作成・削除など、チャート上のあらゆる操作を元に戻す（`Cmd+Z` / `Ctrl+Z`）・やり直す（`Cmd+Shift+Z` / `Ctrl+Y`）ことが可能
...
- **タスク・行の画像管理** — クリップボード（Ctrl+V）からの直接貼り付け、ドラッグ＆ドロップアップロード、自動圧縮、サムネイル表示、ホバーディレイ（500ms）付きポップアッププレビュー（ちらつき防止・ツールチップ競合解消）、ライトボックス拡大表示
- **マイルストーン＆マーカー** — プロジェクトマイルストーン（縦線＋バッジ）および行タイムライン上のマーカー（三角形アイコン＋ラベル）
- **依存関係線 & 削除確認ダイアログ** — タスク間の依存関係を矢印付き曲線（逆方向はS字カーブ）で描画、クリティカルパス（最長チェーン）の自動検出とハイライト。線のクリック選択・削除ボタンおよび右クリックメニューからの削除に対応し、確認ダイアログによる誤削除防止を実装（サマリータスクは集計タスクのため依存線を自動抑止）
- **高画質エクスポート** — 公式エクスポートプラグイン（`@mogura/moguchart-plugin-export`）による PNG 画像および PDF 出力（表示倍率の自動等倍正規化・スクロール位置保持・カレンダー幅拡張時の影補正対応、オンデマンド動的インポートによる初期表示最適化）
- **タイムライン付き Excel エクスポート** — 公式 Excel プラグイン（`@mogura/moguchart-plugin-excel`）による Excel（.xlsx）出力。セル塗りつぶしによる視覚的工程表（タイムライン）とオートフィルター付きデータ一覧テーブルの 2 シート同時出力に対応（オンデマンド動的インポート対応）
...
### 3. データ連携 & バックアップ
- **REST API (OpenAPI 3.1準拠)** — プロジェクト・行・タスク・コメントの個別 CRUD、APIキー認証（発行・失効UI）、レートリミット（60req/分）
...
* **`packages/frontend`**:
  ...
  - **コアコンポーネント**: `@mogura/moguchart-vue` (Vue 3 公式ラッパー) + `@mogura/moguchart-core` (Lit 製 Web Component)
  - **プラグイン**: `@mogura/moguchart-plugin-export` (PNG/PDF 高画質エクスポートプラグイン、オンデマンド動的インポート対応)、`@mogura/moguchart-plugin-excel` (Excel タイムライン付きエクスポートプラグイン、オンデマンド動的インポート対応)
...
### 5. Chrome リモートデバッグツールの利用

E2Eテストやブラウザ自動検証、AIエージェントによる検証用に Chrome をリモートデバッグポート（`9222`）で起動・操作できます。macOS / Windows / Linux の各プラットフォームに対応しています。

```bash
# Chrome をデバッグポート 9222 でバックグラウンド起動
pnpm run chrome:debug
```

また、`scripts/debug-chrome.mjs` を使用して CDP (Chrome DevTools Protocol) 経由でブラウザを操作・検証できます：
...
# 指定 URL を新規タブで開く
node scripts/debug-chrome.mjs open http://localhost:5173/

# 画面のスクリーンショットを保存
node scripts/debug-chrome.mjs screenshot screenshot.png

# ブラウザコンテキストでの JavaScript 実行
node scripts/debug-chrome.mjs eval "document.title"
```
