...
- **高画質エクスポート** — 公式エクスポートプラグイン（`@mogura/moguchart-plugin-export`）による PNG 画像および PDF 出力（表示倍率の自動等倍正規化・スクロール位置保持・カレンダー幅拡張時の影補正対応）
...
* **`packages/frontend`**:
  ...
  - **プラグイン**: `@mogura/moguchart-plugin-export` (PNG/PDF 高画質エクスポートプラグイン)
